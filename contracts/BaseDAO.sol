...
contract BaseDAO {
  uint constant formationPeriod;
  uint constant proposalPeriod;
  uint constant votingPeriod;
  uint constant disputablePeriod;
  ...
  mapping (address => uint) currentDeposits;

  modifier owns(uint _minBalance) {
    require(token.balanceOf(msg.sender) >= _minBalance);
    _;
  }

  function vote(uint _proposalId, bool _isInFavour) payable owns(0);
